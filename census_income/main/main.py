...
#%%
# FAMD lens for categorical and continuous joint factor analysis
# FAMD normalises OHE binary columns using the Greenacre method — centering
# each column by its proportion p_j and scaling by the binomial standard
# deviation — then standardises continuous columns and applies SVD to the
# combined normalised matrix. The first component captures the dominant axis
# of joint variation across all mixed-type variables. Fitted on X_train only
# to prevent data leakage. See helper_funcs.famd_fit() for implementation.
lens_2, famd_ohe_cols, famd_cont_cols, famd_p_j, famd_scaler, famd = (
    helper_funcs.famd_fit(X_train, X_hot_vars)
)

#%%
# FAMD lens projection for validation diagnostics
# Projects the validation set onto the training FAMD axis using the
# normalisation parameters and PCA loadings fitted on X_train. The resulting
# lens_2_val coordinates place validation observations in the same factor
# space as the training graph without re-fitting the Mapper topology.
# See helper_funcs.famd_transform() for projection details.
lens_2_val = helper_funcs.famd_transform(
    X_val, famd_ohe_cols, famd_cont_cols, famd_p_j, famd_scaler, famd
)
...
#%%
# Create 2-D lens with XGBoost Random Forest and FAMD
# lens_1 carries the supervised probability signal from XGBoost: it separates
# high-confidence positive predictions from high-confidence negatives along one
# axis of the cover. lens_2 is the pre-computed FAMD first component, a
# deterministic unsupervised signal derived from the joint factor structure of
# all mixed-type training variables. Pairing a supervised lens with an
# unsupervised one reveals regions where the model's probability surface does
# not align with the underlying data manifold structure, which is where
# systematic failure modes typically concentrate.
...
# simultaneous supervised (XGBoost probability) and unsupervised (FAMD
# component) view of each graph node's composition.
mapper.visualize(
    graph,
    ...
    color_function_name=["XGBoost Random Forest", "FAMD"],
...
# Resets indices to 0-based integers required by Cobalt's internal alignment.
# y_true and y_pred are cast to boolean Categorical columns because
# CobaltDataset expects binary labels in that form to infer the task type.
# Only training data is used so failure groups reflect the same population as
# the Mapper topology.
...
# CobaltDataset wraps the annotated DataFrame with Cobalt's schema layer,
# registering column roles (inputs, target, prediction). add_model declares
# the XGBoost model's I/O contract. compute_model_performance_metrics
# establishes baseline precision, recall, and F1 as the reference for
# evaluating failure group performance drops.
...
# XGBClassifier.apply() returns a (n_train, n_estimators) matrix of leaf node
# indices — nominal identifiers with no inherent ordering or magnitude. Hamming
# distance is the principled metric: it counts the fraction of trees where two
# samples land in different leaves, equal to 1 minus the classical Random Forest
# proximity. add_embedding_array registers the embedding under 'xgbrf_emb' so
# Cobalt's clustering uses the model's own notion of similarity.
xgbrf_emb = xgbrf_model.apply(X_cobalt)
ds.add_embedding_array(xgbrf_emb, metric='hamming', name='xgbrf_emb')

#%%
# Instantiate Cobalt workspace
# DatasetSplit partitions the dataset by index into named splits; passing the
# full X_cobalt index as 'train' exposes all training records to the failure
# group search. Workspace combines the dataset, split, and registered models
# into a single analysis context.
split = cobalt.DatasetSplit(ds, {'train': X_cobalt.index})
w = cobalt.Workspace(ds, split)

#%%
# Find Cobalt failure groups
# Clusters training data by embedding proximity and surfaces sub-groups where
# the error rate exceeds the population average by more than threshold=0.3.
# failure_metric='error' uses binary misclassification as the signal. Results
# are stored under run_name for retrieval in the UI.
...
# Renders the Cobalt browser-based interactive workspace inside a Jupyter or
# VS Code interactive panel. Displays discovered failure groups, their feature
# distributions, performance breakdowns, and comparisons against the full
# population.
w.ui
